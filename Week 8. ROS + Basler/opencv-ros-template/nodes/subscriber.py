...
def image_callback(msg: Image, cv_bridge: CvBridge) -> None:
  ...
  # Формат изображения определяется полем encoding в msg. Если encoding mono8, то изображение чёрно-белое, 1 канал, если цветное - 3
...
def main() -> None:
  ...
  # Этот метод запускает бесконечный цикл ожидания сообщений, что позволяет узлу оставаться активным и обрабатывать входящие сообщения
  rospy.spin()
